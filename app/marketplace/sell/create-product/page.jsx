...
function MediaUpload({ media, setMedia }) {
  ...
  const handleImageUpload = async (files, type) => {
    ...
    setUploadProgress(prev => ({ ...prev, [type]: 1 })); // Start with 1% to show progress bar
    ...
    try {
      ...
      if (type === 'gallery') {
        setMedia(prev => ({
          ...prev,
          gallery: [...prev.gallery, res.url].slice(0, 5)
        }));
      } else {
        setMedia(prev => ({
          ...prev,
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Add images for your product</p>
      </div>

      <div className="space-y-2">
        <Label>Thumbnail <span className="text-red-500">*</span></Label>
        <FileUploader
          onFilesSelected={(files) => handleImageUpload(files, 'thumbnail')}
          acceptedFileTypes={["image/*"]}
          maxFiles={1}
          maxSizeInMB={2}
          disabled={isUploading.thumbnail}
        />
        <UploadProgress progress={uploadProgress.thumbnail} />
        {media.thumbnail && (
          <div className="relative aspect-video w-full max-w-md mt-2">
            <Image
              src={media.thumbnail}
              alt="Thumbnail"
              fill
      ...
      <div className="space-y-2">
        ...
        <FileUploader
          onFilesSelected={(files) => handleImageUpload(files, 'coverPhoto')}
          acceptedFileTypes={["image/*"]}
          maxFiles={1}
          maxSizeInMB={5}
          disabled={isUploading.coverPhoto}
        />
        <UploadProgress progress={uploadProgress.coverPhoto} />
        {media.coverPhoto && (
          <div className="relative aspect-[21/9] w-full max-w-md mt-2">
            <Image
            ...
            <Button
              ...
              size="sm"
              className="absolute top-2 right-2"
              onClick={() => setMedia(prev => ({ ...prev, coverPhoto: null }))}
              disabled={isUploading.coverPhoto}
            >
              <X className="h-4 w-4" />
            </Button>
      ...
      <div className="space-y-2">
        ...
        <FileUploader
          onFilesSelected={(files) => handleImageUpload(files, 'gallery')}
          acceptedFileTypes={["image/*"]}
          maxFiles={1}
          maxSizeInMB={5}
          disabled={isUploading.gallery || media.gallery.length >= 5}
        />
        <UploadProgress progress={uploadProgress.gallery} />
...
function ContentBlock({ block, index, updateBlock, removeBlock, moveBlock, isFirst, isLast }) {
  ...
  const handleFileUpload = async (files) => {
    ...
    setIsUploading(true);
    setUploadProgress(1); // Start with 1% progress
    ...
    try {
      ...
      // Keep at 100% briefly before clearing
...
export default function CreateProductPage() {
  ...
  // Select the appropriate schema based on product type
  const getSchemaForProductType = () => {
    switch(productType) {
      case 'DIGITAL': return digitalProductSchema;
      case 'EBOOK': return ebookProductSchema;
      case 'COURSE': return courseProductSchema;
      case 'EVENT': return eventProductSchema;
      case 'MEMBERSHIP': return membershipProductSchema;
      case 'CALL': return callProductSchema;
      case 'COFFEE': return coffeeProductSchema;
      default: return baseProductSchema;
    }
  }
  ...
  const handleProductTypeSelect = (type) => {
    ...
    // If it's an event product, initialize with empty ticket block
    if (type === 'EVENT') {
      setContentBlocks([{
        type: 'tickets',
        id: Date.now(),
      ...
      // Reset content blocks for other product types
      setContentBlocks([]);
    }
  ...
  // Handle moving to next step without submitting
    ...
    if (isStepValid()) {
      ...
      switch (currentStep) {
        ...
        case 3:
          ...
          break;
        case 4:
          toast.error('Please add at least one content block and fill in all required fields');
          break;
      }
    }
  };

  // Add proper cleanup for file uploads
  useEffect(() => {
    return () => {
      // Cleanup any object URLs to prevent memory leaks
      if (watch('thumbnail')) URL.revokeObjectURL(watch('thumbnail'));
      if (watch('coverPhoto')) URL.revokeObjectURL(watch('coverPhoto'));
    };
  }, [watch]);

  // Improve form validation for EVENT products
  const validateEventProduct = (data) => {
    if (productType === 'EVENT') {
      if (!data.eventDate) return 'Event date is required';
      if (!data.eventLocation) return 'Event location is required';
      if (!data.tickets || data.tickets.length === 0) return 'At least one ticket type is required';
      
      for (const ticket of data.tickets) {
        if (!ticket.name?.trim()) return 'All tickets must have a name';
        if (isNaN(parseFloat(ticket.price))) return 'Invalid ticket price';
        if (isNaN(parseInt(ticket.available)) || parseInt(ticket.available) < 1) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const eventError = validateEventProduct(data);
      if (eventError) {
      ...
      const cleanedData = {
        ...data,
        price: productType === 'EVENT' ? 0 : parseFloat(data.price),
        type: productType,
        content: contentBlocks.map(block => ({
          ...block,
          fileUrl: block.fileUrl?.startsWith('blob:') ? null : block.fileUrl,
        })),
        tickets: productType === 'EVENT' ? data.tickets.map(ticket => ({
          name: ticket.name.trim(),
          price: parseFloat(ticket.price),
          available: parseInt(ticket.available),
          totalCount: parseInt(ticket.available),
          transferable: Boolean(ticket.transferable),
          limitPerBuyer: Boolean(ticket.limitPerBuyer),
          maxPerBuyer: ticket.limitPerBuyer ? Math.min(parseInt(ticket.maxPerBuyer), ticket.available) : 1
        })) : undefined,
        media: {
      ...
      const result = await createProduct.mutateAsync(cleanedData);
      
      // Navigate to the product page on success
      router.push(`/marketplace/products/${result.id}`);
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 1:
        return <ProductTypeSelector onSelect={handleProductTypeSelect} />;
      case 2:
        return (
          <div className="space-y-6">
            <ProductFormFields
  ...
  const isStepValid = () => {
    switch (currentStep) {
      ...
      case 2:
        ...
        if (!formValues.title || formValues.title.length < 3) return false;
        if (!formValues.description || formValues.description.length < 10) return false;
        
        // Price validation for non-EVENT products
        if (productType !== 'EVENT') {
          if (!formValues.price || formValues.price <= 0) return false;
        }
        
        // Event-specific validation
        if (productType === 'EVENT') {
          if (!formValues.eventDate) return false;
          if (!formValues.eventLocation) return false;
          if (!formValues.tickets || formValues.tickets.length === 0) return false;
          
          // Validate each ticket
          const validTickets = formValues.tickets.every(ticket => 
            ticket.name?.trim() &&
            !isNaN(parseFloat(ticket.price)) &&
            !isNaN(parseInt(ticket.available)) &&
            parseInt(ticket.available) > 0
          );
          if (!validTickets) return false;
        }
        
        // Product type specific validation
        switch (productType) {
          case 'EBOOK':
            if (!formValues.format) return false;
            if (!formValues.downloadLink) return false;
            break;
          case 'COURSE':
            if (!formValues.lessons || formValues.lessons.length === 0) return false;
            break;
          case 'MEMBERSHIP':
            if (!formValues.duration) return false;
            if (!formValues.benefits || formValues.benefits.length === 0) return false;
            if (!formValues.subscriptionType) return false;
            break;
          case 'CALL':
            if (!formValues.callDuration || formValues.callDuration < 15) return false;
            if (!formValues.availability) return false;
            break;
        }
      ...
      case 3:
        // Media validation
        return !!media.thumbnail; // Only thumbnail is required
        ...
        return contentBlocks.every(block => {
          switch (block.type) {
            ...
            case 'video':
              return !!block.url;
            case 'question':
              return !!block.question?.trim() && !!block.questionType;
            case 'tickets':
              return block.isRequired || (!!block.eventDate && !!block.eventLocation);
            default:
              return true;
          }
        });
  ...
  // Add this effect to sync ticket data with content blocks
  useEffect(() => {
    if (productType === 'EVENT' && currentStep === 3) {
      const tickets = watch('tickets');
      const eventDate = watch('eventDate');
      const eventLocation = watch('eventLocation');
      
      if (tickets?.length > 0) {
        const ticketBlock = contentBlocks.find(block => block.type === 'tickets');
        if (ticketBlock) {
          const updatedBlock = {
            ...ticketBlock,
            type: 'tickets',
            ...
            ticketTypes: tickets.map(ticket => ({
              ...
              available: parseInt(ticket.available) || 0,
              transferable: ticket.transferable || false,
              limitPerBuyer: ticket.limitPerBuyer || false,
              maxPerBuyer: ticket.limitPerBuyer ? (parseInt(ticket.maxPerBuyer) || 1) : 1
            }))
          };
          
          setContentBlocks(prev => 
            prev.map(block => 
              block.type === 'tickets' ? updatedBlock : block
            )
          );
        } else {
          // If no ticket block exists, create one
          ...
          setContentBlocks(prev => [...prev, newTicketBlock]);
        }
      }
    }
  }, [productType, currentStep, watch('tickets'), watch('eventDate'), watch('eventLocation')]);
  
  useEffect(() => {
    // Simulate checking for any necessary data
  ...
  // Add a new function to handle form submit for draft and publish
  const handleFormSubmit = async (status) => {
    if (!isStepValid()) {
      // Check which step has validation errors
      if (currentStep === 3 && !media.thumbnail) {
        toast.error('Thumbnail image is required');
        return;
      }
    ...
    try {
      ...
      const cleanedData = {
        ...formData,
        price: productType === 'EVENT' ? 0 : parseFloat(formData.price || 0),
        type: productType,
        // status: status, // Comment this out until migration is done
        media: {
          thumbnail: media.thumbnail,
        ...
        tickets: productType === 'EVENT' ? formData.tickets?.map(ticket => ({
          name: ticket.name.trim(),
          price: parseFloat(ticket.price),
          available: parseInt(ticket.available),
          totalCount: parseInt(ticket.available),
  ...
  // Update the FormFooter component
  function FormFooter() {
    return (
