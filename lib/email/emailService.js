...
export const sendVerificationEmail = async (user, verificationToken) => {
  ...
  return sendEmail({
    ...
    data: {
      ...
      verificationUrl,
      appName: process.env.NEXT_PUBLIC_APP_NAME || 'TradeVero',
    },
  });
};
...
export const sendDepositConfirmationEmail = async (user, transaction) => {
  const transactionUrl = `${process.env.NEXT_PUBLIC_APP_URL}/dashboard/transactions`;
  
  // Extract payment method from description or use a default
  let paymentMethod = 'card';
  if (transaction.description) {
    const match = transaction.description.match(/via\s+(\w+(\s+\w+)*)/i);
    if (match && match[1]) {
  ...
  return sendEmail({
    ...
    data: {
      ...
      newBalance: transaction.newBalance || 'Updated',
      transactionUrl,
      appName: process.env.NEXT_PUBLIC_APP_NAME || 'TradeVero',
    },
  });
...
export const sendWithdrawalConfirmationEmail = async (user, transaction, estimatedArrival = '') => {
  ...
  return sendEmail({
    ...
    data: {
      ...
      transactionId: transaction.id,
      transactionDate: transaction.createdAt,
      amount: Math.abs(transaction.amount), // Use absolute value since withdrawal amount is negative
      paymentMethod: paymentMethod,
 ...
 * @returns {Promise} - Promise that resolves with the send info
 */
export const sendOrderCancellationEmail = async (user, order, cancelReason = '') => {
  const orderUrl = `${process.env.NEXT_PUBLIC_APP_URL}/dashboard/orders/${order.id}`;
  ...
  return sendEmail({
    ...
    data: {
      ...
      itemName: order.listing?.username || 'Account',
      amount: order.price,
      cancelDate: order.cancelledAt || new Date(),
