...
export const sendDepositConfirmation = async (userId, transactionId) => {
  try {
    ...
    // Add balance information to the transaction
    const transactionWithBalance = {
      ...transaction,
      newBalance: userBalance ? userBalance.buyingBalance : null
    };

    // Send email notification
    await emailService.sendDepositConfirmationEmail(user, transactionWithBalance);
...
export const sendWithdrawalConfirmation = async (userId, transactionId, estimatedArrival = '') => {
  try {
    ...
    const transactionWithBalance = {
      ...transaction,
      newBalance: userBalance ? userBalance.sellingBalance : null
    };

    // Send email notification
    await emailService.sendWithdrawalConfirmationEmail(user, transactionWithBalance, estimatedArrival);
...
export const sendOrderCancellation = async (userId, orderId, reason = '') => {
  try {
    ...
    // Send push notification
    await pushService.sendOrderPushNotification(userId, order, 'Order Cancelled', 
